...
/**
 * This file defines the interrupt handler for handling the keyboard input
 * which uses a bunch of flags for special keys on the keyboard. It also
 * defines the default keyboard layout(en-us). If you want to change
 * the layout, change the kbdus variable in this file. As far as I know,
 * non-ASCII symbols aren't supported in the VGA standard and we need to 
 * do more stuff if we want to support non-ASCII characters in the future
